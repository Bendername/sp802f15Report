The platform target for this game is mobiles.
...
\begin{itemize}
\item Minimize the space the input interface requires as much as possible, and make the interface intuitive such that it does not draw attention away from the game, see section \ref{introductions:makingthegamefun:removedistractionsfromplayerfocus}.
...
It includes four useable abilities along the right side of the screen, two useable abilities along the bottom of the screen, an \emph{attack} button and a \emph{change target} button.
\begin{figure}[H]
\centering
...
The placement of the action buttons make it such that you do not accidentally hit the wrong buttons. 
