...
On the mobile platform builds we use mobile shaders. These helps with the performance by beying optimized for speed and low performance devices more so than the high end shaders used on normal PCs. We have not considered optimizing the shaders for PC.\tododennis{Considered adding something about the tradeoff for performance e.g. AI / pathfinding is worse, but I don't think it fits...}
